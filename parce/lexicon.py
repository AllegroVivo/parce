...
when a lexicon has only one pattern rule which turns out to be an unambiguous
...
from typing import (
    TYPE_CHECKING, Tuple, Callable, Iterable, Optional, Type,
    Dict, Any, Iterator
)
...
from .ruleitem import (
    RuleItem, evaluate_rule, needs_evaluation, pre_evaluate_rule
)
...
RuleTuple = Tuple[str, "StandardAction", "Target"]
RulesFunc = Callable[["Language"], Iterable[RuleTuple]]
ParseResult = Optional[Tuple[int, str, Optional[re.Match[str]], Optional["StandardAction"], Optional["Target"]]]
...
class LexiconDescriptor:
    ...
    def __init__(
        ...
        """
        self.rules_func: RulesFunc = rules_func    #: the function yielding the rules
        self._re_flags: re.RegexFlag = re_flags
        self._consume: bool = consume
        self._lexicons: Dict[Type[Language], Lexicon] = util.caching_dict(lambda owner: Lexicon(self, owner))  # type: ignore

    def __get__(self, _, owner: Type[Language]) -> Lexicon:
...
class Lexicon:
    ...
    def __init__(
        ...
        self.re_flags: re.RegexFlag = descriptor._re_flags
    ...
    def __call__(self, arg: Optional[Any] = None) -> Lexicon:
    ...
    @util.cached_method
    def _derive(self, arg: Optional[Any]) -> Lexicon:
        """Factory, called when a derived lexicon needs to be created."""
        return Lexicon(self.descriptor, self.language, arg)  # type: ignore - ??? - SP

    def __eq__(self, other: Lexicon) -> bool:
        """Return True if we are the same lexicon or a derivative of the same."""
        if type(other) is type(self):
            return self.descriptor is other.descriptor and self.language is other.language
        return NotImplemented

    def __ne__(self, other: Lexicon) -> bool:
        """Return True if we are not the same lexicon or a derivative of the same."""
        if type(other) is type(self):
            return self.descriptor is not other.descriptor or self.language is not other.language
        return NotImplemented

    @util.cached_property
    def _rules(self) -> Tuple[RuleTuple, ...]:
    ...
    @util.cached_property
    def rules(self) -> Tuple[RuleTuple, ...]:
        """Return all rules in a tuple.
        ...
        """
        return tuple(
            pre_evaluate_rule(rule, self.arg)
            for rule
            in self.descriptor.rules_func(self.language)
            or ()
        )

    def __iter__(self) -> Iterator[RuleTuple]:
        """Yield the rules.
    ...
    def _get_parse_function(self) -> Callable[[str, int], Iterator[ParseResult]]:
        ...
        if not patterns:
            if dynamic_default_action:
                def parse(text: str, pos: int) -> Iterator[ParseResult]:
                ...
                def parse(text: str, pos: int) -> Iterator[ParseResult]:
                    ...
                    if pos < len(text):
                        ...
                        yield pos, text[pos:], None, default_action, None  # type: ignore - default_action is as StandardAction - SP
            elif default_target:
                def parse(text: str, pos: int) -> Iterator[ParseResult]:
                ...
                # just quit parsing
                def parse(_: str, __: int) -> Iterator[ParseResult]:
                ...
                if dynamic_default_action:
                    ...
                    def parse(text: str, pos: int) -> Iterator[ParseResult]:
                        ...
                        while True:
                            ...
                            if i > pos:
                                # noinspection PyUnboundLocalVariable
                                yield pos, text[pos:i], None, default_action, None  # type: ignore - default_action is as StandardAction - SP
                            elif i == -1:
                                break
                            yield i, needle, None, action, target
                            pos = i + l
        ...
        for i, rule in zip(indices, rules):
            if needs_evaluation(rule):
                ...
                action, *target = rule
                static[i] = (action, make_target(self, target))  # type: ignore - 'i' is an int - SP
        ...
        def token(m: re.Match[str]) -> ParseResult:
            """Return pos, text, match, *rule for the match object."""
            return m.start(), m.group(), m, *(static[m.lastindex] or replace(m))  # type: ignore - lastindex is an invalid type? - SP

        def replace(m: re.Match[str]) -> Tuple[Optional[StandardAction], Optional[Target]]:
            """Recursively replace dynamic rule items in the rule pointed to by match object."""
            action, *target = evaluate_rule(dynamic[m.lastindex], m)  # type: ignore - lastindex is an invalid type? - SP
        ...
        if dynamic_default_action:
            ...
            def parse(text: str, pos: int) -> Iterator[ParseResult]:
                ...
                for m in finditer(text, pos):
                    if m.start() > pos:
                        # noinspection PyUnboundLocalVariable
                        yield pos, text[pos:m.start()], None, default_action, None  # type: ignore - default_action is as StandardAction - SP
                    yield token(m)
                    pos = m.end()
                if pos < len(text):
                    yield pos, text[pos:], None, default_action, None  # type: ignore - default_action is as StandardAction - SP
