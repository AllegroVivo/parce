...
class AbstractMutableString:
    ...
    * ``_get_text()`` called by ``__getitem__`` to get a slice or single
    ...
    """
    def __init__(self):  # , text=""): - We aren't using 'text' here, so I've removed it as an argument. - SP
    ...
    def __setitem__(self, key: IntOrSlice, text: str) -> None:
        ...
        if (
            (text or start != end)
            and (end - start != len(text) or self[start:end] != text)
        ):
    ...
    def _apply_changes(self) -> None:
        ...
        if self._changes:
            ...
            added = 0
            end = 0  # For typing compatibility, but it will be set in the loop below.
    ...
    def _get_changes(self) -> Iterable[Change]:
